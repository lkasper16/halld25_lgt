...
#ifndef trdclass_halld25_h
#define trdclass_halld25_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   trdclass_halld25(int RunNum, int MaxEvt, int FirstEvt, int FileNum);
   virtual ~trdclass_halld25();
...
#ifdef trdclass_halld25_cxx
trdclass_halld25::trdclass_halld25(int RunNum_in, int MaxEvt_in=0, int FirstEvt_in=0, int FileNum_in=-1) : fChain(0)
{
  RunNum=RunNum_in;
  MaxEvt=MaxEvt_in;
  ...
  } else { printf("====== trdclass constructor Run=%d  ============\n",RunNum); }
...
// used to generate this class and read the Tree.
  if (tree == 0) {
     char FileName[128];
      ...
      } else { sprintf(FileName,"ROOT/Run_%06d.root",RunNum); }
...
trdclass_halld25::~trdclass_halld25()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t trdclass_halld25::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t trdclass_halld25::LoadTree(Long64_t entry)
{
...
void trdclass_halld25::Init(TTree *tree)
{
...
Bool_t trdclass_halld25::Notify()
{
...
void trdclass_halld25::Show(Long64_t entry)
{
...
Int_t trdclass_halld25::Cut(Long64_t entry)
{
...
std::pair<Double_t, Double_t> trdclass_halld25::TrkFit(TH2F *h2_evt, TF1 &fx, const char *cfx, int rob )
{
  //----------  SF fit ---------------------------
  /*
    h1f->Fit("gaus")
...
//==================================================================
void trdclass_halld25::Count(const char *tit) {
  hcount->Fill(tit,1);
}
void trdclass_halld25::Count(const char *tit, double cut1) {
  char clab[20];
  sprintf(clab,"%s_%.1f",tit,cut1);
  hcount->Fill(clab,1);
}
void  trdclass_halld25::Count(const char *tit, double cut1, double cut2) {
  char clab[20];
...
#endif // #ifdef trdclass_halld25_cxx
